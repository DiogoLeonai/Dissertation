...
//****** Parameters of random numer generator ********************//
#define IA 16807
...
//******   System parameters********************//
#define NN 17324 // 86620      // Amount of Neurons
...
int main(void){
  ...
  int i, j, cont, t, n, contISI, amount_initial_conditions;
  int **listaexc, *conextotalex, **listaini, *conextotalin;
  double tempo, b_IF, g_exc, g_ini, V_reset;
  double kinicial, kfinal, R, real, compl, Rmedio, **nk, *phi, PS;
  int contR, *k, *kmax;
  ...
  //Use the following combinations of (radius , g_exc) for reproducing the waves in the dissertations: (32, 0.15), (55, 0.16) and (75, 0.128)  
  radius = 75.0;
  g_exc = 0.1275;

  /*Creating neurons position*/
  ymax = 976.0;
  Deltax = 7.0;
  Deltay = 8.0;
  Deltaz = 10.0;
  ...
  sprintf(output_filename2, "PS_R%.1f_gexc%.3f_Rg_CV_RL_PS.dat", radius, g_exc);
  q = fopen(output_filename2, "wt");

  sprintf(output_filename, "PHASE_R%.1f_gexc%.3f.dat", radius, g_exc);
  phase = fopen(output_filename, "wt");

  sprintf(output_filename2, "LocalZ_R%.1f_gexc_%.3f.dat", radius, g_exc);
  LocalZ = fopen(output_filename2, "wt");

  //for(g_exc = 0.05; g_exc <= 0.05; g_exc = g_exc + 0.004){
  ...
  for (i = 1; i <= NN; i++){
    ...
    for (t = 1; t <= n; t++){
      ...
      for (i = 1; i <= NN; i++){ // tempo anterior
        ...
        if (x[1 + (i - 1) * N] > -40.0) // modelo de aIEF
        {
          //if(tempo > transient)
            //fprintf(raster, "%.3f \t %i \n", tempo, i);
  ...
  for (i = 1; i <= NN; i = i + 4){
    ...
    fprintf(phase, "%.1f %.1f %.0f", x1[i], y1[i], z1[i]); // positions and V
    phi[i] = 0.0;
    k[i] = 1.0;

    for (tempo = transient; tempo <= n * h; tempo = tempo + 40.0 * h){
      if (kmax[i] > 1)
        if (tempo > nk[k[i]][i] && tempo < nk[kmax[i]][i])
        {
          if (tempo <= nk[k[i] + 1][i])
      ...
      if (tempo >= kinicial && tempo <= 216000.0)
        if (tempo > kfinal - 10000.0)
          fprintf(phase, " %.1f", phi[i]); // positions and V
    }
    fprintf(phase, "\n"); // positions and V
  }
///////////////////////////////////////////////////////////

    //printf("%.2f \t %.4f \t %.3f \t %.3f \t %.3f \t %.1f\n", radius, g_exc, Rmedio / contR, CV, Rmediolocalmedio, PS); // aux : num total conex
    fprintf(q, "%.2f \t %.4f \t %.3f \t %.3f \t %.3f \t %.1f\n", radius, g_exc, Rmedio / contR, CV, Rmediolocalmedio, PS);
 // }
